...
def start():
    ...
    set_cycle = get_integer_input('Capacity는 1, Promotion는 2, PCR은 3입니다. 입력하여 주십시오: ')
    ...
    with tqdm(total=total_files, desc="BI 변환 작업 진행 상황", bar_format="{l_bar}{bar} [파일 {n_fmt}/{total_fmt}]") as pbar:
        for items in filtered_list:
            ...
            back_up(file_path)
            complete = transform_to_pivot(set_cycle, file_path, set_class.sheet_name, set_class.bi_sheet_name)
